...
const e = async ({ subscription, message }) => {
    ...
    if (subscription.queue.current) {
        ...
        if (subscription.queue.current.lengthSeconds === 0)
            ...
            format = new Date(subscription.queue.current.lengthSeconds * 1000).toISOString().substr(14, 5);
        descriptionElements.push('**Сейчас играет**');
        descriptionElements.push(`[${subscription.queue.current.title}](${subscription.queue.current.link})  \`${format}\``);
    }
    if (queue.length === 0) {
        ...
        queue.forEach((t, i) => {
            ...
            if (t.lengthSeconds === 0)
                format = 'Live';
            else if (t.lengthSeconds >= 3600)
                format = new Date(t.lengthSeconds * 1000).toISOString().substr(11, 8);
            else
                format = new Date(t.lengthSeconds * 1000).toISOString().substr(14, 5);
